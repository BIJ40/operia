/**
 * FinancialChartsSection — Donut, Aging Balance, Top Debtors, Recovery Rate, Risk Distribution
 * V2: Added more charts (recovery gauge, risk distribution)
 */
...
function RecoveryRateGauge({ rate }: { rate: number }) {
  const color = rate >= 90 ? 'hsl(142, 76%, 36%)' : rate >= 70 ? 'hsl(45, 93%, 47%)' : 'hsl(0, 72%, 51%)';
  ...
  return (
    <div className="relative">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center pt-6">
        ...
        <span className="text-[10px] text-muted-foreground">recouvrement</span>
      </div>
    </div>
  );
}
...
function RiskDistributionChart({ entities }: { entities: FinancialEntityStats[] }) {
  const counts = { healthy: 0, watch: 0, warning: 0, critical: 0 };
  entities.forEach(e => { counts[e.riskLevel]++; });

  const labels = { healthy: 'Sain', watch: 'Surveillance', warning: 'Alerte', critical: 'Critique' };
  const data = (['critical', 'warning', 'watch', 'healthy'] as const)
    .map(level => ({ name: labels[level], value: counts[level], fill: COLORS_RISK[level] }))
    .filter(d => d.value > 0);

  if (data.length === 0) {
    return <p className="text-sm text-muted-foreground text-center py-12">Aucune donnée</p>;
  }
...
function TopDebtorsList({ entities, limit = 5 }: { entities: FinancialEntityStats[]; limit?: number }) {
...
export function FinancialChartsSection({ analysis, isLoading }: FinancialChartsSectionProps) {
  ...
  const allEntities = [...analysis.byApporteur, ...analysis.byClient];
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card>
          <CardHeader className="pb-2 pt-4 px-4">
            <CardTitle className="text-sm font-semibold">Taux de recouvrement</CardTitle>
          </CardHeader>
          <CardContent className="px-2 pb-3">
            <RecoveryRateGauge rate={analysis.kpis.tauxRecouvrement} />
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="pb-2 pt-4 px-4">
            <CardTitle className="text-sm font-semibold">Répartition des risques</CardTitle>
          </CardHeader>
          <CardContent className="px-2 pb-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent className="px-4 pb-4">
            <TopDebtorsList entities={analysis.byApporteur} />
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="px-4 pb-4">
            <TopDebtorsList entities={analysis.byClient} />
          </CardContent>
        </Card>
      </div>
    </motion.div>
