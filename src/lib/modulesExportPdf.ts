...
export async function exportModulesDocumentationPdf(): Promise<Blob> {
  ...
  addTitle('Sous-modules', 12);
  addBullet('indicateurs - Dashboard principal avec KPIs globaux');
  addBullet('stats_hub - Tableaux de bord avancés');
  addBullet('veille_apporteurs - Alertes apporteurs dormants/en déclin');
  addBullet('actions_a_mener - Liste des actions prioritaires');
  addBullet('diffusion - Affichage sur écran TV');
  addBullet('carte_rdv - Carte interactive des interventions');
  addBullet('mes_apporteurs - Consultation des apporteurs');
  addBullet('gestion_apporteurs - Créer et gérer les comptes apporteurs');
  addTitle('Accès', 12);
  ...
  addBullet('STARTER - Help Academy (apogee), Support (user), Pilotage (indicateurs)');
