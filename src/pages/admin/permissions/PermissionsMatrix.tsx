...
import { Search, User, Users as UsersIcon, HelpCircle } from 'lucide-react';
                ...
                <table className="w-full text-xs">
                  <thead className="sticky top-0 bg-background z-10">
                    <tr className="border-b">
                      ...
                      {scopes?.map(scope => (
                        <th key={scope.id} className="text-center py-2 px-1 font-medium min-w-[60px]">
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <span className="inline-flex items-center gap-1 cursor-help">
                                <span className="max-w-[50px] truncate">{scope.label}</span>
                                <HelpCircle className="w-3 h-3 text-muted-foreground" />
                              </span>
                            </TooltipTrigger>
                            <TooltipContent side="bottom" className="max-w-xs">
                              <p className="font-semibold text-xs mb-2">{scope.label}</p>
                              <div className="space-y-1 text-xs">
                                {[0, 1, 2, 3, 4].map(level => (
                                  <p key={level} className="text-muted-foreground">
                                    <span className="font-medium text-foreground">{level}:</span> {getPermissionHelpText(scope.slug, level as PermissionLevel)}
                                  </p>
                                ))}
                              </div>
                            </TooltipContent>
                            ...
                            <td key={scope.id} className="text-center py-2 px-1">
                              <Tooltip>
                                ...
                                <TooltipContent className="max-w-xs">
                                  <p className="font-semibold text-xs mb-1">{scope.label} – Niveau {result.level}</p>
                                  <p className="text-xs text-muted-foreground mb-1">
                                    {getPermissionHelpText(scope.slug, result.level as PermissionLevel)}
                                  </p>
                                  <p className="text-xs text-muted-foreground/70">Source: {result.source}</p>
                                </TooltipContent>
                              </Tooltip>
                            </td>
                          );
                          ...
                          {scopes?.map(scope => {
                            ...
                            return (
                              <td key={scope.id} className="text-center py-2 px-1">
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Badge className={cn("text-xs cursor-help", PERMISSION_LEVELS[level]?.color)}>
                                      {level}
                                    </Badge>
                                  </TooltipTrigger>
                                  <TooltipContent className="max-w-xs">
                                    <p className="font-semibold text-xs mb-1">{scope.label} – Niveau {level}</p>
                                    <p className="text-xs text-muted-foreground">
                                      {getPermissionHelpText(scope.slug, level as PermissionLevel)}
                                    </p>
                                  </TooltipContent>
                                </Tooltip>
                              </td>
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              {PERMISSION_LEVELS.map(level => (
                <Tooltip key={level.value}>
                  <TooltipTrigger asChild>
                    <Badge className={cn("text-xs cursor-help", level.color)}>
                      {level.value} = {level.label}
                    </Badge>
                  </TooltipTrigger>
                  <TooltipContent className="max-w-xs">
                    <p className="font-semibold text-xs mb-1">Niveau {level.value} – {level.label}</p>
                    <p className="text-xs text-muted-foreground">
                      {level.value === 0 && "Aucun accès au module."}
                      {level.value === 1 && "Consultation en lecture seule."}
                      {level.value === 2 && "Création et modification de contenu."}
                      {level.value === 3 && "Gestion complète du périmètre."}
                      {level.value === 4 && "Administration et configuration du module."}
                    </p>
                  </TooltipContent>
                </Tooltip>
              ))}
