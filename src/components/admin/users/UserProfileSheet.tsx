  ...
  const { data: rpcModules, isLoading: rpcModulesLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (Array.isArray(data)) {
        for (const row of data as Array<{ module_key: string; granted: boolean; access_level: string; options: unknown }>) {
          if (!row.granted || row.access_level === 'none') continue;
          ...
          result[row.module_key] = { enabled: true, options: opts };
        }
      }
      return result;
    },
