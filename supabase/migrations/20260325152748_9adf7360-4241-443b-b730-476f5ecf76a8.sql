...
-- =============================================================================
-- MIGRATION: Remove legacy_plan_modules from get_user_effective_modules
-- Unify permissions: single source of truth = module_registry + user_modules
-- =============================================================================

-- STEP 1: Data cleanup
-- 1a. Delete orphan rows (no match in module_registry)
DELETE FROM plan_tier_modules
WHERE module_key NOT IN (SELECT key FROM module_registry);

-- 1b. Delete rows for non-deployed modules
DELETE FROM plan_tier_modules
WHERE module_key IN (
  SELECT key FROM module_registry WHERE is_deployed = false
);

-- 1c. Delete rows for NONE (Individuel) modules
DELETE FROM plan_tier_modules
WHERE module_key IN (
  SELECT key FROM module_registry WHERE required_plan = 'NONE'
);

-- STEP 2: Rewrite RPC — remove legacy_plan_modules entirely
...
BEGIN
  ...
  WITH RECURSIVE deployed_tree AS (
    -- Build the tree with inherited deployment status
  ...
  -- SINGLE SOURCE: registry + plan_tier_modules (no legacy path)
  registry_modules AS (
  ...
  -- User-level overrides (individual assignments via user_modules)
  ...
  -- Ancestor grants: if user has child override, grant parent access
  ancestor_grants AS (
  ...
  -- Merge: registry (plan-based) + user overrides + ancestor grants
  merged AS (
    SELECT 
      rm.module_key,
