...
-- 1) Add missing legacy module keys to module_registry with correct min_role
INSERT INTO module_registry (key, label, node_type, min_role, is_deployed, required_plan, parent_key)
VALUES 
  ('agence', 'Pilotage agence', 'section', 2, true, 'STARTER', NULL),
  ('rh', 'Ressources humaines', 'section', 2, true, 'PRO', NULL),
  ('parc', 'Parc véhicules & EPI', 'section', 2, true, 'PRO', NULL),
  ('divers_plannings', 'Plannings', 'feature', 2, true, 'STARTER', NULL),
  ('divers_reunions', 'Réunions', 'feature', 2, true, 'STARTER', NULL),
  ('divers_documents', 'Documents', 'feature', 2, true, 'STARTER', NULL),
  ('divers_apporteurs', 'Apporteurs', 'section', 2, true, 'PRO', NULL),
  ('prospection', 'Commercial / Prospection', 'section', 1, true, 'PRO', NULL)
ON CONFLICT (key) DO UPDATE SET min_role = EXCLUDED.min_role;

-- 2) Fix the RPC: add min_role filter to legacy_plan_modules CTE
