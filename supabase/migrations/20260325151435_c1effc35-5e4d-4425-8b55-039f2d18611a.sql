-- Fix: legacy_plan_modules CTE must exclude modules with required_plan = 'NONE'
-- Also clean up stale plan_tier_modules rows for NONE modules
...
BEGIN
  ...
  legacy_plan_modules AS (
    ...
    WHERE ptm.tier_key = v_tier_key
      ...
      AND COALESCE(mr.required_plan, 'STARTER') != 'NONE'
  ),
...
-- Clean up stale plan_tier_modules rows for modules marked as NONE (Individuel)
DELETE FROM plan_tier_modules ptm
USING module_registry mr
WHERE ptm.module_key = mr.key
  AND mr.required_plan = 'NONE';