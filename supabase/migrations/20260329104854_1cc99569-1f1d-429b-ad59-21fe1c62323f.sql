...
BEGIN
  ...
  resolved AS (
    ...
    ORDER BY
      ...
      CASE mod_source
        ...
        WHEN 'agency_delegation'   THEN 2
        WHEN 'platform_assignment' THEN 2
        WHEN 'pack_grant'          THEN 2
        WHEN 'job_preset'          THEN 2
  ...
  not_granted_mods AS (
    SELECT
      mc.key                  AS mod_key,
      false                   AS mod_granted,
      'none'::text            AS mod_access_level,
      '{}'::jsonb             AS mod_options,
      mc.node_type            AS mod_node_type,
      'not_granted'::text     AS mod_source,
      false                   AS mod_preconditions_ok
    FROM module_catalog mc
    WHERE mc.is_deployed = true
      AND mc.key NOT IN (SELECT mod_key FROM resolved)
      AND mc.key NOT IN (SELECT mod_key FROM section_mods)
  )
  ...
  SELECT
    ng.mod_key, ng.mod_granted, ng.mod_access_level,
    ng.mod_options, ng.mod_node_type, ng.mod_source, ng.mod_preconditions_ok
  FROM not_granted_mods ng;
