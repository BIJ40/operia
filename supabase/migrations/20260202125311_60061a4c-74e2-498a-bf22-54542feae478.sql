-- Fix the track_entity_changes trigger function to handle employment_contracts correctly
-- The issue is that it tries to access first_name/last_name which don't exist on this table
...
BEGIN
  ...
  -- Générer label lisible - FIXED: handle tables that don't have first_name/last_name
  IF TG_OP = 'DELETE' THEN
    v_entity_label := CASE TG_TABLE_NAME
      ...
      WHEN 'employment_contracts' THEN 'Contrat ' || OLD.contract_type
      WHEN 'salary_history' THEN 'Salaire'
      ELSE NULL
    END;
  ...
  ELSE
    v_entity_label := CASE TG_TABLE_NAME
      ...
      WHEN 'employment_contracts' THEN 'Contrat ' || NEW.contract_type
      WHEN 'salary_history' THEN 'Salaire'
      ELSE NULL
    END;
