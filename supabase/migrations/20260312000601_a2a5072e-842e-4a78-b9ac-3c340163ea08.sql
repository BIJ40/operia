-- ============================================================================
-- PHASE 6 — RPC Stabilization Migration
-- 
-- BLOC 1: Align module_registry metadata (6 UPDATEs)
-- BLOC 2: Replace get_user_effective_modules (registry_modules CTE only)
--
-- Idempotent. Safe to re-run.
-- Rollback: supabase/rollback/phase6_rollback_rpc.sql
-- ============================================================================

-- ============================================================================
-- BLOC 1: Align module_registry metadata
-- ============================================================================

UPDATE module_registry SET required_plan = 'PRO' WHERE key = 'pilotage.dashboard';
UPDATE module_registry SET min_role = 2 WHERE key = 'organisation.salaries';
UPDATE module_registry SET min_role = 2 WHERE key = 'organisation.parc';
UPDATE module_registry SET min_role = 2 WHERE key = 'organisation.apporteurs';
UPDATE module_registry SET min_role = 2 WHERE key = 'organisation.reunions';
UPDATE module_registry SET min_role = 2 WHERE key = 'mediatheque.documents';

-- ============================================================================
-- BLOC 2: Replace RPC get_user_effective_modules
-- Only the registry_modules CTE changes (LEFT JOIN plan_tier_modules).
-- All other CTEs are verbatim identical to the previous version.
-- ============================================================================
...
BEGIN
  ...
  -- PHASE 6 FIX: LEFT JOIN plan_tier_modules to carry options and respect enabled flag
  registry_modules AS (
