...
BEGIN
  ...
  -- NEW: Recursive expansion of user overrides to deployed children
  user_override_expanded AS (
    -- Direct overrides from user_modules
    SELECT 
      um.module_key,
      true AS enabled,
    ...
    -- Recursively include deployed children of overridden parent keys
    SELECT 
    ...
    WHERE mr.is_deployed = true
      ...
      -- Skip children that have their own direct override
      AND NOT EXISTS (
  ...
  user_overrides AS (
    -- Direct overrides take priority over inherited
